...
# Initialize an empty list to hold all of our tweets
tweets = []
...
#------------------------------------------------------------------------------------------------#
with codecs.open("./newtwitter.csv", "r", encoding='utf-8', errors='ignore') as csvfile:
#------------------------------------------------------------------------------------------------#
    ...
    for row in reader:
        ...
        tweet['clean'] = re.sub(r'http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+', '',
                                tweet['clean'])
...
for tweet in negative_tweets[0:100]:
    print("id=%s, polarity=%.2f, clean=%s" % (tweet['id'], tweet['polarity'], tweet['clean']))
